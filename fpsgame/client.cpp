...
namespace game
{
    ...
    extern int newhud_itemspos_centerfirst;
    ...
    void checkseserverinfo() {
        ...
        if(lastpreviewdata.lastteamupdate + SERVUPDATEINTERVAL < totalmillis &&
           lastpreviewdata.lastupdate + SERVUPDATETEAMGAP < totalmillis) {
    ...
    void drawteammates(fpsent *d, float x, float y, float s)
    {
        ...
        loopv(players) 
        {
            ...
            if(o != d && o->state == CS_ALIVE && isteam(o->team, d->team))
            {
                if(!alive++) 
                {
                    settexture(isteam(d->team, getcurrentteam()) ? "packages/hud/blip_blue_alive.png" : "packages/hud/blip_red_alive.png");
                    glBegin(GL_QUADS);
                }
        ...
        loopv(players) 
        {
            ...
            if(o != d && o->state == CS_DEAD && isteam(o->team, d->team))
            {
                if(!dead++) 
                {
                    settexture(isteam(d->team, getcurrentteam()) ? "packages/hud/blip_blue_dead.png" : "packages/hud/blip_red_dead.png");
                    glBegin(GL_QUADS);
                }
                drawteammate(d, x, y, s, o, scale);
            }
    ...
    static bool playersentrysort (const playersentry& p1, const playersentry& p2) {
